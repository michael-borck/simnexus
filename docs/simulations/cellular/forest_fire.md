...
**Outputs.** `run_simulation()` advances the grid for `days` generations and returns a `List[float]` of the number of trees at each generation (it also populates `tree_history` and `fire_history` with the per-generation tree and burning-cell counts). `get_statistics()` summarises the run with `mean_trees`, `mean_fires`, `max_fires`, and `final_tree_fraction` (final tree count divided by total cells). See [`forest_fire_simulation.py`](https://github.com/michael-borck/sim-lab/blob/main/src/sim_lab/core/forest_fire_simulation.py) for the source.
